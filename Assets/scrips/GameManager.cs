...
public class GameManager : MonoBehaviour
{
    ...
    // 열거형의 자료를 설정하는 정의문 나열의 자료는 0, 1, 2.....의 값이 할당됨
    public enum STATE
    {
    ...
    IEnumerator MakeStage()
    {
        ...
        // 카드배열 읽기 배열의 1행을 읽고 변수 t에 할당함
        string[] str = StageSet.stage[stageNum - 1];

        // 배열의 행의 수만큼 반복
        foreach (string t in str)
        {
            // 각행의 문자열을 단일 문자 배열로 변환(문자열 좌우의 공백 제거), 변수t의 좌우 공백을 제거(Trim)하고 단일 문자배열로 변환
            ...
            // 1행의 문자열 길이만큼  반복
            // 배열의 ch의 한문자를 읽고 변수 c에 할당함
            foreach (char c in ch)
            {
                switch (c)
                {
                    // 맵의 내용이 * 이면 그 위치에 카드를 만들어서 배치함
                    case '*':
                      // 카드 만들기
        ...
        void SetCardPos (out float sx, out float sz)
        {
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                {
                    // 문자열(string)은 단일 문자(char)의 배열로 취급할 수 있다
                    switch (t[j])
                    {
                        case '.':
                        case '*':
